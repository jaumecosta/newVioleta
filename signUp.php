<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Formulario</title>
    <link rel="stylesheet" href="_assets/css/style.css">
</head>
<body>
<div class="ladoizquierdo">
        <img src="_assets/img/taza.svg" alt="la_poderosa_taza">
        <p class="left_text">Plan your activities and control your progress online</p>
    <img src="_assets/img/Grupo 1.svg" alt="avion_despegando">
</div>
<div class="formulario">
            <section>
                <a href="index.php" class="enlace1">Sign In</a>
                <a href="signUp.php" class="enlace2">Sign Up</a>
                <h1 class="signar">Sign in or Sign Up</h1>
            <form action="xd.php" class="formulario">
        <input type="text" placeholder="Enter your full name" class="casilla">
        <input type="text" placeholder="contraseña" class="casilla">
        <input type="text" placeholder="Your email goes here" class="casilla">
        <div class="checkbox">
        <input type="checkbox" class="botoncito" require>I agree all statements in <a href="#" class="enlace">terms os service</a>
        </div>
        <button type="submit" class="boton">Sign Up</button>
        <a href="index.php" class="enlace">I'm not a member yet</a>
        </form>
        </section>
</div>
</body>
</html>